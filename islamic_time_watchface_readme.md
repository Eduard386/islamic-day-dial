...
- В продуктовой модели `Isha` продолжается до `Fajr`, а `Last Third` является вложенным выделенным интервалом внутри `Isha`.

### 3.6. Fajr Safety Cutoff
...
- название текущей фазы (Maghrib, Isha, Last Third, Fajr, Duha, Dhuhr, Asr);
- дата по Хиджре;
...
7. Last Third выделяется корректно;
8. web dashboard показывает кольцо и debug data;
9. dashboard деплоится на GitHub Pages;
10. есть unit tests и snapshots;
...
- **7 сегментов**: maghrib_to_isha, isha_to_last_third, last_third_to_fajr, fajr_to_sunrise, sunrise_to_dhuhr, dhuhr_to_asr, asr_to_maghrib.
- **Рендер**: 480 sub-arcs с solid цветами (sweep), без полос и швов.
- **Цветовая схема** (`segment-gradients.ts`):
  - **Ночь** (isha_to_last_third, last_third_to_fajr): чёрный `#000000`.
...
- **Название сегмента** — Maghrib, Isha, Last 3rd, Fajr, Duha, Dhuhr, Asr.
...
- **Ночь** (Isha→Last 3rd, Last 3rd→Fajr) — луна: тёмно-оранжевый градиент `#f5e6c8`→`#e8c878`→`#d4a04a`, highlight `#f0d8a0`, обводка `#c9a04a`.
